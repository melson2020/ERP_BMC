...
public interface IProducePlanRepository extends JpaRepository<ProducePlan,Integer> {
    ...
    @Query(nativeQuery = true,value = "SELECT pp.id,pp.planNo,pp.orderFormNo,pp.createDate,pp.startDate,pp.endDate,pp.pickingTicketNo,pt.state as pickingState,pp.state as planState FROM `produce_plan` pp RIGHT JOIN picking_ticket pt on pp.pickingTicketNo=pt.ticketNo WHERE pp.startDate='2';")
