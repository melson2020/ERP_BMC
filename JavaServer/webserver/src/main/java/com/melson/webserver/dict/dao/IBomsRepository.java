...
public interface IBomsRepository extends JpaRepository<Boms, Integer> {
    ...
    @Query(value = "SELECT b.id,b.bomNo,b.partNo,p.`name`,b.chPartNo,p.unit,b.chQty,b.supplyName,p.specification FROM `boms` b RIGHT JOIN product p ON b.chPartNo=p.productNo WHERE b.bomNo in ?1 ORDER BY b.sindex", nativeQuery = true)
