...
public interface IPurchaseOrderRepository extends JpaRepository<PurchaseOrder,String> {
    ...
    @Query(nativeQuery = true, value = "SELECT po.id,po.poNo,po.state,po.supplyId,po.deliverDay,po.payterm,po.createDate,po.createBy,po.description ,su.`name` as supplyName FROM `purchase_order` po left JOIN supply su on po.supplyId=su.id where po.state=?1")
